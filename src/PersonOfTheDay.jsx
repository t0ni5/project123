...
function PersonOfTheDay({ name, info , portrait, index}){
  ...
  switch (index){
    ...
    return(
        ...
        <Card.Img variant="top" src={portrait} alt="burglar's portrait" className='img-fluid max-width-100 mx-auto text-center'/>
        </Col>
        
        <Col lg={8} md={8} >
        
        <Card.Title className="caption"  id="daily_name">{name}</Card.Title> 
        <Card.Text className="description ">
          {info}
        </Card.Text>
        <div className="text-center">
              <Button variant="primary" className='read_more' href={pathname}>{t('read_button.read_more')}</Button>
            </div>
        </Col>
      </Card.Body>
